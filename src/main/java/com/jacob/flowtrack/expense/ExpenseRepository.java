...
public interface ExpenseRepository extends JpaRepository<Expense, Long> {
    ...
    @Query("SELECT e FROM Expense e WHERE e.user = :user AND (:category IS NULL OR LOWER(e.category) = LOWER(:category)) AND (:min IS NULL OR e.amount >= :min) AND (:max IS NULL OR e.amount <= :max)")
    ...
    @Query("SELECT e.category AS category, SUM(e.amount) AS total FROM Expense e WHERE e.user = :user GROUP BY e.category")
    List<CategorySummary> getCategorySummary(@Param("user") User user);

    @Query("SELECT EXTRACT(YEAR FROM e.createdAt) AS year, EXTRACT(MONTH FROM e.createdAt) AS month, SUM(e.amount) AS total FROM Expense e WHERE e.user = :user GROUP BY EXTRACT(YEAR FROM e.createdAt), EXTRACT(MONTH FROM e.createdAt) ORDER BY EXTRACT(YEAR FROM e.createdAt), EXTRACT(MONTH FROM e.createdAt)")
    List<MonthlySummary> getMonthlySummary(@Param("user") User user);
    ...
    List<Expense> findByWorkspaceInAndStatus(List<Workspace> workspaces, ExpenseStatus status);

    @Query("SELECT e FROM Expense e JOIN WorkspaceMember wm ON wm.workspace = e.workspace WHERE wm.user = :user AND e.status = :status")
    List<Expense> findPendingForUserWorkspaces(@Param("user") User user, @Param("status") ExpenseStatus status);
