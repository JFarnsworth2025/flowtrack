...
import com.jacob.flowtrack.expense.comments.ExpenseCommentResponse;
import com.jacob.flowtrack.expense.comments.ExpenseCommentService;
import com.jacob.flowtrack.expense.receipt.ExpenseReceiptService;
import com.jacob.flowtrack.expense.receipt.ReceiptResponse;
...
public class ExpenseController {
    ...
    @GetMapping("/search")
    public ResponseEntity<ApiResponse<PaginatedResponse<ExpenseResponse>>> searchExpenses(@AuthenticationPrincipal CustomUserDetails userDetails, @RequestParam Long workspaceId, @RequestParam(required = false) String keyword, @RequestParam(required = false) String category, @RequestParam(required = false) BigDecimal min, @RequestParam(required = false) BigDecimal max, @RequestParam(required = false) LocalDateTime startDate, @RequestParam(required = false) LocalDateTime endDate, @RequestParam(required = false) ExpenseStatus status, @RequestParam(required = false) String fullName, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
    ...
    public ResponseEntity<ApiResponse<List<ReceiptResponse>>> getReceiptForExpense(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long id) {
        User user = customUserDetails.getUser();
        List<ReceiptResponse> receipts = receiptService.getReceiptsForExpense(id, user);
